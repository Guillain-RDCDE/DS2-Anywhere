...
- 📖 **[docs/](docs/)** — a long-form, didactic walkthrough of:
  ...
  - **(5)** [**📄 the re-sync excitation anomaly — research paper**](docs/09-the-resync-excitation-anomaly.md) — the format's last unsolved secret, written up in full: how *analysis-by-synthesis* proved the decoder's filter is flawless and the bug is entirely in the excitation, nine falsified hypotheses, the smoking-gun frame dissection, the hidden state machine, and the exact architectural wall that blocks the final inch. Didactic enough for a newcomer, precise enough to resume from. **The one we haven't beaten — yet.**
...
- **the open case** → [📄 docs/09 — the re-sync excitation anomaly](docs/09-the-resync-excitation-anomaly.md) — the one bug still standing, as a full research paper: the analysis-by-synthesis proof, nine falsified hypotheses, the hidden state machine, and the wall.
