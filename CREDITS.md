...
**And once more, the other direction — [PR #16](https://github.com/hirparak/dss-codec/pull/16).** Patrick folded two fixes from his FFmpeg work back into the Rust crate. One half — a combinatorial-index overflow fix — was clean and we kept it. The other rewrote the QP demux and removed the `byte1` re-sync anchor, garbling paused recordings. We shadow-benched the PR against 316 real files, sent him structure-only reproducers (block framing kept, every trace of voice erased), and flagged a separate `usize` panic in the SP demux with a one-line fix (`end.saturating_sub(spos)`). Patrick traced his side to a set of golden masters that had been corrupted by a stray test decode, pulled the demux rewrite, kept the codec fix, took the panic patch, and merged. The story is [chapter 15](docs/15-the-relay-runs-backward.md).
