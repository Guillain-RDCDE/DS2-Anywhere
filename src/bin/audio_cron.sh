...
# Two modes per pass:
#
#   MODE 1 (legacy queue) — consume AUDIO_ROOT/audio_aconv/
...
#
#   MODE 2 (auto-feed from mail/) — scan active commands in the database
#     - SELECT message_id WHERE subject has .ds2/.dss and status is active and <= 7 days
#     - look in AUDIO_ROOT/mail/m{prefix}/{cmd_id}/ for top-level *.ds2/*.dss
...
SRC=/srv/AUDIO_ROOT/ftpsaisieaudio/faudio/audio_aconv
DST_MAIL_BASE=/srv/AUDIO_ROOT/ftpmadactylo/mail
DST_FTP_BASE=/srv/AUDIO_ROOT/ftpsaisieaudio/41
DST_STT_BASE=/srv/AUDIO_ROOT/ftpsaisieaudio/tmp_audio_speechtotext

LOG_DIR=/var/log
LOG="$LOG_DIR/audio_converter.log"
mkdir -p "$LOG_DIR"
...
log "=== audio_cron pass (dryrun=$DRYRUN) ==="

# ============================================================
# Helpers
# ============================================================

# Compute output stem (MP3 name) from the source basename.
compute_stem() {
  local fname="$1"
  local cmd_id="$2"
  local stem
...
# Convert src -> $TMPDIR/<stem>.mp3, distribute to the 3 destinations.
# args: src cmd_id stem label
# returns 0 on success, 2 on encryption, 3 on convert fail, 4 on distribute fail.
convert_and_distribute() {
  ...
  if [ -n "$encryption" ] && [ "$encryption" != "none" ]; then
    ...
    {
      echo "Command $cmd_id: encrypted DS2 ($encryption) detected by audio_cron ($label)"
      echo "Source: $src"
      echo "Needs password — handle via the admin convertisseur page"
    } | mail -s "[CONV-DS2] $cmd_id encrypted ($label)" "$ALERT_TO" 2>/dev/null || true
    return 2
  fi

  local t0=$(date +%s)
  if ! "$CLI" "$src" "$out_mp3" >> "$LOG" 2>&1; then
    log "[$cmd_id $label] CONVERSION FAILED"
    {
      echo "Command $cmd_id: DS2 conversion failed ($label)"
      echo "Source: $src"
      echo "Detail in $LOG"
    } | mail -s "[CONV-DS2] $cmd_id FAIL ($label)" "$ALERT_TO" 2>/dev/null || true
    return 3
  fi
  local t1=$(date +%s)
  ...
  for dest in "$dest_mail" "$dest_ftp" "$dest_stt"; do
    if ! mkdir -p "$dest"; then
      log "[$cmd_id $label] mkdir $dest failed"
      return 4
    fi
    chmod 777 "$dest" 2>/dev/null || true
    if ! cp "$out_mp3" "$dest/${stem}.mp3"; then
      log "[$cmd_id $label] cp to $dest failed"
      return 4
    fi
...
if [ ${#aconv_files[@]} -eq 0 ]; then
  ...
  for f in "${aconv_files[@]}"; do
    ...
    # DB status — if delivered/cancelled → leftover, just cleanup
    ...
    # Anti-doublon: if destination MP3 is fresh (typically produced by mode 2
    # a few seconds before conv_fast.sh dropped the DS2 in audio_aconv), skip
    # the reconversion and just clean up. 10-min window is wide enough to
    # absorb mode2/mode1 timing skew but narrow enough to NOT confuse with
    # a stale MP3 from yesterday.
    prefix="${cmd_id::-4}"
...
# ============================================================
# MODE 2: auto-feed from mail/  (the new way)
# ============================================================
...
if [ -z "$cmds" ]; then
  ...
  for cmd_id in $cmds; do
    ...
    # Look for top-level DS2/DSS (NOT _audio_remplaces_)
    ds_files=( "$D"/*.ds2 "$D"/*.dss )
    if [ ${#ds_files[@]} -eq 0 ]; then
      n_feed_noop=$((n_feed_noop+1)); continue
    fi

    # Already an audio file next to it? -> skip (idempotent)
