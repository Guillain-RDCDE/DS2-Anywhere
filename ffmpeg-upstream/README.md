...
| DSS SP synthesis instability (report, not a patch) | ⚠️ **The cause reported here is wrong — see the 2026-08-30 row.** **Sent to the v3 thread 2026-08-26** (SMTP 250), `In-Reply-To` the v3 cover `<20260720171555.3156820-1-guillain@poulpe.us>`. Reports that `dss_sp.c`'s direct-form LPC synthesis goes unstable on long DS-7000 recordings (frame 2420 / t=58 s on our reference file), that precision is *not* the cause (DLL tables and an f64 rewrite both reproduce it), and that the Olympus DLL uses a lattice filter instead. Includes the energy-limiter stopgap we ship, explicitly offered as a limiter and not proposed for merge. Body: [`patches/email-body-v3-followup-dss-sp-agc.txt`](patches/email-body-v3-followup-dss-sp-agc.txt); story: [`docs/16`](../docs/16-the-q15-instability.md). |

| DSS SP lattice synthesis patch | ⚠️ **Withdrawn.** Sent 2026-08-27 (patchwork [60515](https://patchwork.ffmpeg.org/patch/60515/), [60517](https://patchwork.ffmpeg.org/patch/60517/)), rebuilding `dss_sp.c`'s synthesis as a lattice. The diagnosis was wrong: the direct form was never the problem, the demuxer was handing it misframed data. Asked for both to be marked rejected on 2026-08-30. Kept here as the honest record; story: [`docs/17`](../docs/17-the-framing-was-wrong.md). |
| **DSS SP framing + sample rate (the actual fix)** | **Sent 2026-08-30** (SMTP 250), then resent as **v2** the same day after the first send went out under a bare address with no name on it — `git am` takes the mail's `From:` as the commit author, so that would have landed nameless in FFmpeg's history. Two patches: [`patches/v2-dss-sp-0001-*.patch`](patches/v2-dss-sp-0001-set-the-sample-rate-to-11000.patch) sets the rate to 11000 Hz (the codec runs at 12000 and decimates 11:12); [`patches/v2-dss-sp-0002-*.patch`](patches/v2-dss-sp-0002-take-framing-from-the-block-headers.patch) takes the framing from each block header instead of running on from the first frame. Reference bench 0.5849 → 0.9995, `fate-dss-sp` and `fate-dss-lp` unchanged and bit-identical, no regression across 187 recordings. Also supersedes [59867](https://patchwork.ffmpeg.org/patch/59867/) (the June paused-recording patch), which fixed one case of the same bug. Body: [`patches/email-body-dss-sp-framing-and-rate.txt`](patches/email-body-dss-sp-framing-and-rate.txt); story: [`docs/17`](../docs/17-the-framing-was-wrong.md). |
